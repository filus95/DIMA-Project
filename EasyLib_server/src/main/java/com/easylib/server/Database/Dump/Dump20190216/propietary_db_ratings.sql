...
INSERT INTO `ratings` VALUES ('1626366543',2,1),('8830430714',6.66667,3);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `on_update_rating_propDB` AFTER UPDATE ON `ratings` FOR EACH ROW begin
    update library_1.books set averageRating = round(NEW.rating)
    where books.identifier = NEW.book_identifier;

    update library_2.books set averageRating = round(NEW.rating)
    where books.identifier = NEW.book_identifier;
